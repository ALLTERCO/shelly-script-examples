...
// CHANGE HERE
function onButtonPress(BTHparsed, BLEData) {
...
let BTHomeDecoder = {
    ...
    getUInt24LE: function (buffer) {
        return (
            0x00ffffff &
            ((buffer.at(2) << 16) | (buffer.at(1) << 8) | buffer.at(0))
        );
    },
    ...
    unpack: function (buffer) {
        ...
        while (buffer.length > 0) {
            let _bth = BTH[buffer.at(0)];
            if (_bth === "undefined") {
                console.log('BTH: unknown type');
                return null;
            }
            buffer = buffer.slice(1);
            let _value = this.getBufValue(_bth.t, buffer);
            if (_value === null) return null;
            if (typeof _bth.f !== "undefined") _value = _value * _bth.f;
            result[_bth.n] = _value;
            this.buffer = buffer.slice(getByteSize(_bth.t));
        }
        return result;
    },
};
