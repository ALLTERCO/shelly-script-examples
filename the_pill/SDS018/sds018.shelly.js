...
/**
 * Nova Fitness SDS018 PM2.5/PM10 Air Quality Sensor for Shelly
 *
 * Reads particulate matter (PM2.5 and PM10) concentrations using the
 * SDS018 laser dust sensor via UART.
 *
 * Hardware connection:
 * - SDS018 TX (Pin 5) -> Shelly RX (GPIO)
 * - SDS018 RX (Pin 4) -> Shelly TX (GPIO)
 * - VCC (Pin 1) -> 5V (4.7-5.3V, >1W)
 * - GND (Pin 3) -> GND
 *
 * Note: SDS018 uses 3.3V TTL logic levels for UART.
 *
 * Protocol:
 * - Baud rate: 9600
...
function init() {
    print("Nova Fitness SDS018 Air Quality Sensor");
    print("Initializing...");
    ...
    Timer.set(2000, false, function() {
        ...
        if (CONFIG.WORK_PERIOD > 0) {
            Timer.set(100, false, function() {
                SDS018.setWorkPeriod(CONFIG.WORK_PERIOD);
            });
        }
    });
}
