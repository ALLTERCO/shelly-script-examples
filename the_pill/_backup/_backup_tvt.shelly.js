...
const BAUD = 9600;
const ADDR = 0xfa; // universal address (recommended)const uart = UART.get();

const VC_RX = Virtual.getHandle('text:200'); // optional: shows last RXconst VC_TX = Virtual.getHandle('text:201'); // input commandconst VC_BTN = Virtual.getHandle('button:200'); // POWER btn
...
function buildFraion buildFrame(triple) {
 dFrame(triple) {
   return [ADDR, 0xf1, triple[0] & 0xff, triple[1] & 0xff, triple[2] & 0xff];
}

// RX formatting helpersfunction toHexLine(data) {  let hex = '';  for (let i = 0; i < data.length; i++) {    const b = data.charCodeAt(i) & 0xff;    hex += (i ? ' ' : '') + toHexByte(b);  }  return hex;}
...
// UART initif (!uart.configure({ baud: BAUD, mode: '8N1' })) {  die();}

print('YS-IRTM ready @', BAUD, '8N1');
print('Commands: POWER | VOL+ | VOL- | MUTE');

// RX debug: print HEX + JS arrayuart.recv(function (data) {  if (!data || !data.length) return;  // Optional: ignore ACK F1  if (data.length === 1 && (data.charCodeAt(0) & 0xff) === 0xf1) {    return;  }  VC_RX.setValue(toHexLine(data) + ' -> ' + ��return;  }  VC_RX.setValue(toHexLine(data) + ' -> ' + toJsArray(data));});

funcX.setValue(toHexLine(data) + ' -> ' + toJsArray(data));});
