 ...
 *   charger values.
 * @status production
 * @link https://github.com/ALLTERCO/shelly-script-examples/blob/main/power-energy/victron-mppt-solar-controller_vc.shelly.js
 */
...
var VIRTUAL_COMPONENTS = {
  components: [
    { key: 'chargeState', type: 'text', id: 200, config: { name: 'Charge State', default_value: '', persisted: false, meta: { ui: { view: 'label', maxLength: 64 }, cloud: ['log'] } } },
    { key: 'chargerError', type: 'text', id: 201, config: { name: 'Charger Error', default_value: '', persisted: false, meta: { ui: { view: 'label', maxLength: 128 }, cloud: ['log'] } } },
    { key: 'batteryVoltage', type: 'number', id: 200, config: { name: 'Battery Voltage', default_value: 0, min: 0, max: 100, meta: { ui: { view: 'label', unit: 'V', step: 0.01 }, cloud: ['measurement'] } } },
    { key: 'batteryChargingCurrent', type: 'number', id: 201, config: { name: 'Battery Charging Current', default_value: 0, min: -100, max: 100, meta: { ui: { view: 'label', unit: 'A', step: 0.01 }, cloud: ['measurement'] } } },
    { key: 'yieldToday', type: 'number', id: 202, config: { name: 'Yield Today', default_value: 0, min: 0, max: 100000, meta: { ui: { view: 'label', unit: 'Wh', step: 1 }, cloud: ['measurement'] } } },
    { key: 'solarPower', type: 'number', id: 203, config: { name: 'Solar Power', default_value: 0, min: 0, max: 100000, meta: { ui: { view: 'label', unit: 'W', step: 1 }, cloud: ['measurement'] } } },
    { key: 'externalDeviceLoad', type: 'number', id: 204, config: { name: 'External Device Load', default_value: 0, min: -100, max: 100, meta: { ui: { view: 'label', unit: 'A', step: 0.01 }, cloud: ['measurement'] } } }
  ],
  groups: [
    { id: 200, name: 'Victron MPPT', components: ['chargeState', 'chargerError', 'batteryVoltage', 'batteryChargingCurrent', 'yieldToday', 'solarPower', 'externalDeviceLoad'] }
  ]
};

var vcChargeState = null;
var vcChargerError = null;
var vcBatteryVoltage = null;
var vcBatteryChargingCurrent = null;
var vcYieldToday = null;
var vcSolarPower = null;
var vcExternalDeviceLoad = null;

function bindVirtualComponents(readyVc) {
  vcChargeState = readyVc.handles.chargeState;
  vcChargerError = readyVc.handles.chargerError;
  vcBatteryVoltage = readyVc.handles.batteryVoltage;
  vcBatteryChargingCurrent = readyVc.handles.batteryChargingCurrent;
  vcYieldToday = readyVc.handles.yieldToday;
  vcSolarPower = readyVc.handles.solarPower;
  vcExternalDeviceLoad = readyVc.handles.externalDeviceLoad;
}
