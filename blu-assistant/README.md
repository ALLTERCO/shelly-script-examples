...
> **Note:**
> Demo scripts that use Virtual Components now create, verify, and group their own UI fields/buttons at startup. Extend each script's `VIRTUAL_COMPONENTS` manifest when adding new UI controls.

## Table of Contents

1. [Demo Example Scripts](#1-demo-example-scripts)
...
2. [Additional Example Scripts](#2-additional-example-scripts)
   - [Factory Reset](#21-factory-reset-factory-reset-devicejs)
   - [Gen3 Matter Update](#22-gen3-matter-update-gen3-update-matterjs)
   - [Inventory Labels with Printer](#23-inventory-labels-with-printer-print-label-onlinejs)
   - [BLU Door/Window -> Webhook Bridge](#24-blu-doorwindow-webhook-bridge-bthome-webhookjs)
3. [Physical Button Scripts (two-script pair)](#3-physical-button-scripts-two-script-pair)
   - [Button Event Source](#31-button-event-source-button-event-sourcejs)
   - [WiFi Provisioning by Model ID](#32-wifi-provisioning-by-model-id-wifi-provisioningjs)
...
## 1. Demo Example Scripts

The demo scripts below create and verify their own Virtual Components before attaching button handlers or reading UI field values.



### 1.1 Wi-Fi Provisioning (add-to-wifi.shelly.js)
...
### 2.2 Gen3 Matter Update (gen3-update-matter.shelly.js)
...
### 2.3 Inventory Labels with Printer (print-label-online.shelly.js)
...
### 2.4 BLU Door/Window -> Webhook Bridge (bthome-webhook.shelly.js)
...
## 3. Physical Button Scripts (two-script pair)
...
### 3.1 Button Event Source (button-event-source.shelly.js)
...
### 3.2 WiFi Provisioning by Model ID (wifi-provisioning.shelly.js)
...
4. For the physical-button pair: start **both** `button-event-source` and `wifi-provisioning`, then press the hardware button to trigger a scan.
5. **Trigger** each demo script by its virtual or physical button.

-

## 5. Customization

- Add or remap virtual components in each script's `VIRTUAL_COMPONENTS` manifest.
...
- **Missing UI fields?** Restart the script and check its `VIRTUAL_COMPONENTS` manifest for the expected fields.
